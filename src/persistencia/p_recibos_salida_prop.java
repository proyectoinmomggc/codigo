 ...
 * @author Gonzalo
 */
public class p_recibos_salida_prop {
    ...
    public static void guardarrecibo(d_recibo rec) throws Exception {
        ...
        try {
            pst = c.prepareStatement("Update recibos_salida_prop set detalle=?, fecha=?, nombre_inq=?, importe=? Where idmovimiento=?");
            ...
            if (resultado == 0) {
                ...
                pst = c.prepareStatement("Insert Into recibos_salida_prop Values (?,?,?,?,?)");
    ...
    public static void eliminarrecibos() throws Exception {
        ...
        try {
            ...
            st.execute("delete from recibos_salida_prop");
        } catch (Exception ex) {
            throw new Exception(ex.getMessage());
        }
        c.close();
